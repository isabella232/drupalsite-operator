...
func (r *DrupalSiteReconciler) ensureResources(drp *webservicesv1a1.DrupalSite, log logr.Logger) (transientErr reconcileError) {
	...
	// This will be created by the operator's helm chart and deleted from here
	if transientErr := r.ensureResourceX(ctx, drp, "is_base", log); transientErr != nil {
		return transientErr.Wrap("%v: for Site builder base ImageStream")
	}
	...
	// might remove, see https://gitlab.cern.ch/webservices/webframeworks-planning/-/issues/244
	if transientErr := r.ensureResourceX(ctx, drp, "is_drush", log); transientErr != nil {
		return transientErr.Wrap("%v: for Drush ImageStream")
	}
	...
	// might remove, see https://gitlab.cern.ch/webservices/webframeworks-planning/-/issues/244
	if transientErr := r.ensureResourceX(ctx, drp, "bc_drush", log); transientErr != nil {
		return transientErr.Wrap("%v: for Drush BuildConfig")
	}
	...
	// 3. Serving layer
	// Note: nginx will become a sidecar in the PHP deployment, see https://gitlab.cern.ch/drupal/paas/drupalsite-operator/-/issues/25

	if transientErr := r.ensureResourceX(ctx, drp, "dc_php", log); transientErr != nil {
		return transientErr.Wrap("%v: for PHP-FPM DC")
	}
	if transientErr := r.ensureResourceX(ctx, drp, "svc_php", log); transientErr != nil {
		return transientErr.Wrap("%v: for PHP-FPM SVC")
	}
	if transientErr := r.ensureResourceX(ctx, drp, "fpm_cm", log); transientErr != nil {
		return transientErr.Wrap("%v: for PHP-FPM CM")
	}
	if transientErr := r.ensureResourceX(ctx, drp, "dc_nginx", log); transientErr != nil {
...
func baseImageReferenceToUse(d *webservicesv1a1.DrupalSite) string {
	if len(d.Spec.Environment.ExtraConfigRepo) > 0 {
		return imageStreamForDrupalSiteBuilderS2I(d).Name + ":" + d.Spec.DrupalVersion
	}
	return imageStreamForDrupalSiteBuilderBase(d).Name + ":" + d.Spec.DrupalVersion
}

// imageStreamForDrupalSiteBuilderBase returns a ImageStream object for Drupal SiteBuilder S2I
// If this is created by the operator helm chart in a given namespace, necessary code changes have to be made
func imageStreamForDrupalSiteBuilderBase(d *webservicesv1a1.DrupalSite) *imagev1.ImageStream {
	ls := labelsForDrupalSite(d.Name)
	ls["app"] = "site-builder"
	objectName := "drupal-site-builder-base-" + d.Name
...
// imageStreamForDrupalSiteDrush returns a ImageStream object for Drupal Nginx
func imageStreamForDrupalSiteDrush(d *webservicesv1a1.DrupalSite) *imagev1.ImageStream {
	ls := labelsForDrupalSite(d.Name)
	ls["app"] = "drush"
	is := createImageStream("drupal-drush-"+d.Name, d.Namespace, ls)
	addOwnerRefToObject(is, asOwner(d))
	return is
}
			...
			CommonSpec: buildv1.CommonSpec{
				...
				Strategy: buildv1.BuildStrategy{
					SourceStrategy: &buildv1.SourceBuildStrategy{
						From: corev1.ObjectReference{
							Kind: "ImageStreamTag",
							Name: "drupal-site-builder-base-" + d.Name + ":" + d.Spec.DrupalVersion,
						},
					},
				},
					...
					Images: []buildv1.ImageSource{
						{
							From: corev1.ObjectReference{
								Kind: "ImageStreamTag",
								Name: baseImageReferenceToUse(d),
								// "drupal-site-builder-s2i-" + d.Name + ":latest",
							},
...
// buildConfigForDrupalSiteDrush returns a BuildConfig object for Drupal Drush tooling
func buildConfigForDrupalSiteDrush(d *webservicesv1a1.DrupalSite) *buildv1.BuildConfig {
	ls := labelsForDrupalSite(d.Name)
	ls["app"] = "drush"
					...
					Images: []buildv1.ImageSource{
						{
							...
							Paths: []buildv1.ImageSourcePath{
								{
									SourcePath:     "/app",
									DestinationDir: "./images/drush/drupal-files/",
								},
								{
									SourcePath:     "/usr/bin/composer",
									DestinationDir: "./images/drush/drupal-files/",
								},
							},
						},
	...
	dep := &appsv1.DeploymentConfig{
		...
		Spec: appsv1.DeploymentConfigSpec{
			...
			Template: &corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
					Containers: []corev1.Container{{
						Image:           objectName + ":" + d.Spec.DrupalVersion,
						Name:            "nginx",
						ImagePullPolicy: "Always",
						Ports: []corev1.ContainerPort{{
							ContainerPort: 8080,
...
func (r *DrupalSiteReconciler) ensureResourceX(ctx context.Context, d *webservicesv1a1.DrupalSite, resType string, log logr.Logger) (transientErr reconcileError) {
	switch resType {
	case "is_base":
		res := imageStreamForDrupalSiteBuilderBase(d)
	...
	case "is_php":
		...
		return createResource(ctx, res, res.Name, res.Namespace, r, log)
	case "is_drush":
		res := imageStreamForDrupalSiteDrush(d)
		return createResource(ctx, res, res.Name, res.Namespace, r, log)
	case "bc_s2i":
	...
	case "bc_php":
		...
		return createResource(ctx, res, res.Name, res.Namespace, r, log)
	case "bc_drush":
		res := buildConfigForDrupalSiteDrush(d)
		return createResource(ctx, res, res.Name, res.Namespace, r, log)
	case "dc_mysql":
