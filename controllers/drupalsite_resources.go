...
func deploymentForDrupalSite(currentobject *appsv1.Deployment, dbodSecret string, d *webservicesv1a1.DrupalSite, drupalVersion string) error {
	...
	currentobject.Spec.Template.Spec.Containers[0].Name = "nginx"
	currentobject.Spec.Template.Spec.Containers[0].ImagePullPolicy = "IfNotPresent"
	currentobject.Spec.Template.Spec.Containers[0].Ports = []corev1.ContainerPort{{
		ContainerPort: 8080,
		Name:          "nginx",
		Protocol:      "TCP",
	}}
	currentobject.Spec.Template.Spec.Containers[0].Env = []corev1.EnvVar{
		{
			Name:  "DRUPAL_SHARED_VOLUME",
			Value: "/drupal-data",
		},
	}
	currentobject.Spec.Template.Spec.Containers[0].EnvFrom = []corev1.EnvFromSource{
		{
	...
	currentobject.Spec.Template.Spec.Containers[0].VolumeMounts = []corev1.VolumeMount{
		{
			Name:      "drupal-directory-" + d.Name,
	...
	currentobject.Spec.Template.Spec.Containers[0].Resources = nginxResources

	currentobject.Spec.Template.Spec.Containers[1].Name = "php-fpm"
	currentobject.Spec.Template.Spec.Containers[1].ImagePullPolicy = "IfNotPresent"
	currentobject.Spec.Template.Spec.Containers[1].Ports = []corev1.ContainerPort{{
		ContainerPort: 9000,
		Name:          "php-fpm",
		Protocol:      "TCP",
	}}
	currentobject.Spec.Template.Spec.Containers[1].Env = []corev1.EnvVar{
		{
			Name:  "DRUPAL_SHARED_VOLUME",
			Value: "/drupal-data",
		},
	}
	currentobject.Spec.Template.Spec.Containers[1].EnvFrom = []corev1.EnvFromSource{
		{
			SecretRef: &corev1.SecretEnvSource{
				LocalObjectReference: corev1.LocalObjectReference{
					Name: dbodSecret,
	...
	currentobject.Spec.Template.Spec.Containers[1].VolumeMounts = []corev1.VolumeMount{
		{
			Name:      "drupal-directory-" + d.Name,
			MountPath: "/drupal-data",
		},
